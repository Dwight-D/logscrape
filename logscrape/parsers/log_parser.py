...
class LogParser:
    ...
    def __init__(self, file_path):
        self.path = file_path
        self.file_map = open_file_map(file_path)
        self.encoding = 'utf-8'
        self.cursor_index = 0
        self.match_index = 0
        self.matches = []
    ...
    def create_entry(self, first_index, last_index):
        """Take first and last known matching indices and find the corresponding log entry. Insert into self.entries"""

        last_index = self.find_next_newline(last_index)
        self.entries.append(Entry(first_index, last_index))
    ...
    def readline(self, index=None):
        ...
        return self.file_map.readline().decode(self.encoding)
